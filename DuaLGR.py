...
from utils import load_data, normalize_weight, bundled_label2origin, get_n_clusters, pca_weights
...
files_syn = [
    ...
    'PolarDB-for-PostgreSQL-2_src_include_access_merge_h',
    'PolarDB-for-PostgreSQL-4_src_include_access_merge_h',
    'PolarDB-for-PostgreSQL-6_src_include_access_merge_h',
    'PolarDB-for-PostgreSQL-8_src_include_access_merge_h',
...
# for file in files_real:
    ...
    parser.add_argument('--weight_soft', type=int, default=0.8, help='smooth-sharp paramter')
    ...
    matrix = np.load('D:/Document/研究生/research/graph clustering/data/usage/' + file + '.npy', allow_pickle=True)
    N = matrix.shape[0]
    ...
    if train:
        ...
        if is_real == False:
            eva(y, bundled_label2origin(file, y_pred, N), 'Kz')
        else:
            eva_real(bundled_label2origin(file, y_pred, N), file)
        print()
        ...
        for epoch_num in range(epoch):
            ...
            for v in range(graph_num):
                ...
                a = eva(bundled_label2origin(file, y_prim, N), bundled_label2origin(file, y_pred, N), visible=False, metrics='nmi')
                best_a[v] = a
        ...
        # =========================================evaluation=============================================================
            if epoch_num % update_interval == 0:
                ...
                with torch.no_grad():
                    ...
                    if is_real == False:
                        nmi, acc, ari, f1 = eva(y, bundled_label2origin(file, y_eval, N), str(epoch_num) + 'Kz')
                        nmi_list.append(nmi.item())
                        acc_list.append(acc.item())
                    else:
                        nmi, acc, ari, f1 = 0, 0, 0, 0
                        modularity = eva_real(bundled_label2origin(file, y_eval, N), file)
                        modularity_list.append(modularity.item())
            if is_real == False:
                if ari + nmi > best_ari + best_nmi:
                    ...
                    bad_count = 0
                    best_result = bundled_label2origin(file, y_eval, N)
                    ...
                    with open('./result/staged/results_syn.txt', 'a') as f:
                        f.write(str(file) + ', {:.4f}'.format(acc) + ', {:.4f}'.format(f1) + \
                            ', {:.4f}'.format(ari) + ', {:.4f}'.format(nmi) + '\n')
                    with open('./result/staged/' + file + '.txt', 'w') as f:
                ...
                if modularity > threshold:
                    ...
                    with open('./result/staged/results_test.txt', 'a') as f:
                        f.write(str(file) + ', ' + str(args.n_clusters) +', {:.4f}'.format(modularity) + '\n')
                    suffix = str(modularity).split('.')[1][:4]
                    with open('./result/staged/real/test/' + file + '-' + str(args.n_clusters) + '-' + str(suffix) + '.txt', 'w') as f:
                        for item in best_result:
                            f.write(str(item) + ' ')
                    test_cnt += 1
                else:
    ...
    with torch.no_grad():
        ...
        kmeans = KMeans(n_clusters=class_num, n_init=5)
        y_eval = kmeans.fit_predict(z_all[-1].detach().cpu().numpy())
        y_eval = bundled_label2origin(file, y_eval, N)
        if is_real == False:
            nmi, acc, ari, f1 = eva(y, y_eval, 'Final Kz')
            with open('./result/staged/ablation/results_semantic.txt', 'a') as f:
                f.write(str(file) + ', {:.4f}'.format(acc) + ', {:.4f}'.format(f1) + \
                    ', {:.4f}'.format(ari) + ', {:.4f}'.format(nmi) + '\n')
            with open('./result/staged/ablation/semantic/' + file + '.txt', 'w') as f:
                for item in best_result:
                    f.write(str(item) + ' ')
            # with open('./result/weights.txt', 'a') as f:
            #     f.write(str(file) + ': ' + str(weights) + '\n')
        else:
            modularity = eva_real(y_eval, file)
            # with open('./result/staged/results_real.txt', 'a') as f:
            #     f.write(str(file) + ', ' + str(args.n_clusters) +', {:.4f}'.format(modularity) + '\n')
            # with open('./result/staged/real/test/' + file + '-' + str(args.n_clusters) + '.txt', 'w') as f:
            #     for item in best_result:
            #         f.write(str(item) + ' ')

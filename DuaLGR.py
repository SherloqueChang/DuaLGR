...
for file in files_real:
    ...
    # n_clusters = get_n_clusters(file)
        ...
        parser.add_argument('--endecoder_lr', type=float, default=5e-5, help='learning rate for autoencoder')
            ...
            # =========================================evaluation=============================================================
                if epoch_num % update_interval == 0:
                    ...
                    with torch.no_grad():
                        ...
                        if is_real == False:
                            nmi, acc, ari, f1 = eva(y, bundled_label2origin(file, y_pred, N), str(epoch_num) + 'Kz')
                            nmi_list.append(nmi.item())
                            acc_list.append(acc.item())
                        else:
                            nmi, acc, ari, f1 = 0, 0, 0, 0
                            modularity = eva_real(bundled_label2origin(file, y_pred, N), file)
                ...
                if is_real == False:
                    if acc > best_acc:
                        ...
                        with open('./result/bundle/results_test.txt', 'a') as f:
                            f.write(str(file) + ', {:.4f}'.format(acc) + ', {:.4f}'.format(f1) + \
                                ', {:.4f}'.format(ari) + ', {:.4f}'.format(nmi) + '\n')
                        with open('./result/bundle/' + file + '.txt', 'w') as f:
                    ...
                    if modularity > best_modularity:
                        ...
                        # with open('./result/bundle/' + file + '-' + str(args.n_clusters) + '.txt', 'w') as f:
                        #     for item in best_result:
                        #         f.write(str(item) + ' ')
                    else:
                        bad_count += 1
        ...
        with torch.no_grad():
            ...
            if is_real == False:
                ...
                # with open('./result/bundle/results_syn.txt', 'a') as f:
                #     f.write(str(file) + ', {:.4f}'.format(acc) + ', {:.4f}'.format(f1) + \
                #         ', {:.4f}'.format(ari) + ', {:.4f}'.format(nmi) + '\n')
                # with open('./result/bundle/' + file + '.txt', 'w') as f:
                #     for item in best_result:
                ...
                with open('./result/bundle/test/results_real.txt', 'a') as f:
                    f.write(str(file) + ', ' + str(args.n_clusters) +', {:.4f}'.format(modularity) + '\n')
                with open('./result/bundle/test/real/' + file + '-' + str(args.n_clusters) + '.txt', 'w') as f:
                    for item in best_result:
                        f.write(str(item) + ' ')
